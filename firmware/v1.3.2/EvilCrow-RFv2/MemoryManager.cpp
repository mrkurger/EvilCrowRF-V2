...
void MemoryPool::mergeBlocks(Block *block)
{
...
MemoryPool::MemoryPool(size_t total, size_t block) : totalSize(total), blockSize(block)
{
...
bool MemoryManager::canAllocate(size_t size, uint32_t caps)
{
    return heap_caps_get_largest_free_block(caps) >= size;
}
