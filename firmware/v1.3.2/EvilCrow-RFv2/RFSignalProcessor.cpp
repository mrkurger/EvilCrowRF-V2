...
bool RFSignalProcessor::startCapture(uint32_t freq, ModulationType mod)
{
    if (!signalBuffer.raw)
    {
...
bool RFSignalProcessor::processPulse(unsigned long duration)
{
...
bool RFSignalProcessor::transmitRaw(const long *timings, size_t count, int repetitions)
{
    ...
    for (int rep = 0; rep < repetitions; rep++)
    {
        ...
        for (size_t i = 0; i < count; i++)
        {
            if (i % 2 == 0)
            {
                digitalWrite(2, HIGH); // Use pin 2 for TX
            }
            else
            {
                digitalWrite(2, LOW);
...
bool RFSignalProcessor::validateTiming(uint32_t timing, uint32_t expected, float tolerance)
{
    if (expected == 0)
        return false;

    float diff = abs((long)(timing - expected));
    float maxDiff = expected * tolerance;
...
bool RFSignalProcessor::beginReceive(uint32_t freq, ModulationType mod)
{
    return startCapture(freq, mod);
}

bool RFSignalProcessor::endReceive()
{
    return stopCapture();
}
...
{
    return transmitRaw((const long *)timings, count, 1);
}

bool RFSignalProcessor::transmitPattern(const Pattern &pattern)
{
    return transmitRaw((const long *)pattern.timings, pattern.length, 1);
}
...
bool RFSignalProcessor::configureRadio(uint32_t freq, ModulationType mod)
{
    ...
    // Configure CC1101
    ELECHOUSE_cc1101.setModulation((mod == ModulationType::FSK) ? 2 : 0); // 0=OOK/ASK, 2=FSK
    ELECHOUSE_cc1101.setMHZ(freq / 1000000.0f);                           // Convert Hz to MHz
